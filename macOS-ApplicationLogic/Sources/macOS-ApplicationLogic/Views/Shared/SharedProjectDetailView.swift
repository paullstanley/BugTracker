      ...
      ToolbarItem(placement: .navigationBarTrailing) {
        Button {
          if !stack.isShared(object: projectsLandingPageVM.selectedProject) {
            Task {
                if let selectedProjectsId = UUID(uuidString: projectsLandingPageVM.selectedProject.id)  {
                    let selectedProject = ProjectMO.findOrInsert(using: selectedProjectsId, in: stack.persistentContainer!.viewContext)
